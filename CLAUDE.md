...
성능 개선 작업은 반드시 의사결정 과정을 문서화한다 (`docs/load-test/` 디렉토리).

- **대안 비교**: 검토한 방법들을 표로 정리하고 각각의 장단점을 명시한다.
- **선택 이유**: 왜 특정 방법을 선택했는지 근거를 남긴다.
- **설계 변경 이력**: 같은 부분이라도 라운드별로 판단이 달라지면 왜 바뀌었는지 기록한다 (예: 인덱스 컬럼 추가/제거 이유).
- **측정 기반**: EXPLAIN 결과, 부하 테스트 수치, Grafana 지표를 근거로 사용한다. 추측이 아닌 데이터로 판단한다.
- **Before/After**: `docs/load-test/performance-report.md`에 라운드별 핵심 지표 비교를 누적한다.
