...
### 환불해줘야 하는 경우의 수
1. 후보자 등록 신청 후 결제 완료 → 후보자 등록 신청 거절
2. 후보자 등록 신청 후 결제 완료 -> 관리자 승인 전 사용자 후보자 등록 신청 철회
3. 매칭 신청 후 결제 완료 → 매칭 신청 거절
4. 매칭 신청 후 결제 완료 -> 관리자 승인 전 사용자 매칭 신청 철회
5. 서비스 장애 등으로 인한 환불 (관리자 수동 처리)

### 환불이 불가능 한 경우
1. 후보자 등록 신청 후 결제 완료 → 관리자 승인 후 사용자 후보자 등록 신청 철회 (관리자 승인 후에는 후보자 등록 신청이 확정되므로 환불 불가)
2. 매칭 신청 후 결제 완료 → 관리자 승인 후 사용자 매칭 신청 철회 (관리자 승인 후에는 매칭 신청이 확정되므로 환불 불가)
3. 사용자 단순 변심에 의한 환불 (서비스 이용 약관에 명시된 환불 정책에 따라 처리)
